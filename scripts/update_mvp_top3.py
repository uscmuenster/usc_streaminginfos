...
def _format_ranking(
    ...
    wert1 = row_map.get("Wert1", "").strip()
    wertung = row_map.get("Wertung", "").strip()

    if not wertung and "Wertung" in headers:
        wertung_index = headers.index("Wertung")
        if wertung_index < len(original_row):
            wertung = original_row[wertung_index].strip()
